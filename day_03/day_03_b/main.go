...
func main() {
	...
	y := 0
	allSyms := make([]symbol, 0)
	allNbrs := make([]number, 0)
	for scanner.Scan() {
		line := scanner.Text()
		p := newParser(line, y)
	...
	for _, s := range allSyms {
		touchingNbrs := make([]number, 0)
		for _, n := range allNbrs {
			if s.touches(n) {
				touchingNbrs = append(touchingNbrs, n)
			}
		...
		if len(touchingNbrs) == 2 {
			sum += touchingNbrs[0].n * touchingNbrs[1].n
		}
	}
